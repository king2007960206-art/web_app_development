from flask import Blueprint, render_template, request, jsonify, session, flash, redirect, url_for
from app.services.ai_service import ask_question

voice_bp = Blueprint('voice', __name__)

@voice_bp.before_request
def require_login():
    """如果是 API endpoint 回傳 401，一般頁面則重導向。"""
    if 'user_id' not in session:
        if request.path.startswith('/api/'):
            return jsonify({'error': 'Unauthorized'}), 401
        flash('請先登入！', 'warning')
        return redirect(url_for('auth.login'))

@voice_bp.route('/voice/qa', methods=['GET'])
def qa_page():
    """提供使用者用語音問答交流的聊天室畫面，主要會載入相關的 Web API 控制腳本於 voice/qa.html。"""
    return render_template('voice/qa.html')

@voice_bp.route('/api/voice/ask', methods=['POST'])
def api_voice_ask():
    """
    供前端非同步 Fetch 存取的內部 API 端點。
    接收夾帶使用者對話字串的 JSON，呼叫外部 AI 後，回傳含有答覆文的 JSON 以供 TTS 發聲。
    """
    data = request.get_json()
    if not data or 'prompt' not in data:
        return jsonify({'status': 'error', 'message': 'Missed prompt parameters'}), 400
        
    prompt = data.get('prompt')
    answer = ask_question(prompt)
    if not answer:
         answer = "抱歉，我的大腦好像卡住了，請再一次發問！"
         
    return jsonify({
        'status': 'success',
        'answer': answer
    })
